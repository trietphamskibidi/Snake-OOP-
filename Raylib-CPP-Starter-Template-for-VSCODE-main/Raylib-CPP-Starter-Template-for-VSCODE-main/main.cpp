...
differentstages stages=gameinitialize;
enum differentstages
{
 gameinitialize,
 gamerunning,
  ...
  void grow()
{
...

  {
    for (int i=1;i<bodysegment.size(); i++)
     ...
     {
       bodysegment={Vector2{20,21},Vector2{20,22},Vector2{20,23}};
...
class gamestage
{
    public:
     ...
     void gamepause()
     {
        DrawText("click anywhereto start again", 300, 450,25, ORANGE);
     }
};
int main () {
    snake body;
    ...
    while (WindowShouldClose() == false)
    {
        ...
        if(gamestage==gamerunning)
        {
            Stage.gamerunning(direction,pendingDirection,body,Food);
            if(IsKeyPressed(KEY_ESCAPE))
            {
                gamestage=gamepausing;
            }
            
        }
        if(gamestage==gamepausing)
        {
            Stage.gamepause();
